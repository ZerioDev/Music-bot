...
module.exports = {
  ...
  async execute({ inter }) {
    ...
    if (!track && !number)
      return inter.editReply({
        content: await Translate(
          `You have to use one of the options to jump to a song <${inter.member}>... try again ? <❌>`
        ),
      });
    ...
    if (track) {
      ...
      if (!skipTo)
        return inter.editReply({
          content: await Translate(
            `Could not find <${track}> <${inter.member}>... try using the url or the full name of the song ? <❌>`
          ),
        });
      ...
      if (!name)
        return inter.editReply({
          content: await Translate(
            `This track does not seem to exist <${inter.member}>... try again ? <❌>`
          ),
        });
