...
module.exports = {
    ...
    async execute({ inter }) {
        ...
        if (!track && !number) inter.editReply({ content: `You have to use one of the options to remove a song ${inter.member}... try again ? ❌` });

        let trackName;

        if (track) {
            const toRemove = queue.tracks.toArray().find((t) => t.title === track || t.url === track);
            if (!toRemove) return inter.editReply({ content: `could not find ${track} ${inter.member}... try using the url or the full name of the song ? ❌` });
        ...
        const embed = new EmbedBuilder()
            .setColor('#2f3136')
            .setAuthor({ name: `Removed ${trackName} from the queue ✅` });
