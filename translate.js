...
module.exports = {
  Translate: async (text = '', lang = '', allUnderScore = false) => {
    ...
    if (lang === "en") output = text;
    else {
      const regex = /<([^>]+)>/g;
      const arrayStr = text.split(regex);
      const translatedArray = await Promise.all(
        arrayStr.map(async (str, index) => {
          if (index % 2 === 0) {
            if (verifyLang(lang)) {
                try {
                    return !allUnderScore ? await translate(str, lang) : await translate(str, lang).toUnderscore();
                } catch (e) {
                    return str;
                }
            } else {
...
function verifyLang(lang) {
    const langs = ["af", "sq", "ar", "hy", "eu", "be", "bn", "bg", "ca", "hr", "cs", "da", "nl", "en", "et", "fi", "fr", "gl", "ka", "de", "el", "gu", "he", "hi", "hu", "is", "id", "ga", "it", "ja", "kn", "ko", "lo", "la", "lv", "lt", "mk", "ms", "ml", "mt", "mr", "mn", "ne", "no", "fa", "pl", "pt", "ro", "ru", "sa", "sr", "si", "sk", "sl", "es", "sw", "sv", "ta", "te", "th", "tr", "uk", "ur", "vi", "cy", "yi", "zu"];
